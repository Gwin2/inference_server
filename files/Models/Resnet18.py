...
def post_func_resnet18(src='', model_type='', model_id=0, ind_inference=0, data=None, prediction=None, mapping=None):
    try:
        fig, ax = plt.subplots()
        ax.imshow(data[0], cmap='gray')
        plt.savefig(src + 'inference_' +  + str(mapping[int(prediction)]) + '_real_' + str(model_id) + '_' + str(model_type) + '.png')
        fig, ax = plt.subplots()
