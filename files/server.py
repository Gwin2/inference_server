...
def receive_data():
    ...
    result_msg = {'message': 'Data inference successfully', 'prediction_1': classes[int(prediction_1)],
                  'prediction_2': classes[int(prediction_2)], 'prediction_3': classes[int(prediction_3)]}
...
def build_model():
    ...
    lin = torch.nn.Sequential(torch.nn.Conv2d(2, 3, kernel_size=(7, 7), stride=(2, 2), padding=(3, 3), bias=False),
                              model_1.conv1)
    model_1.conv1 = lin
...
def run_example():
    ...
    print()
    for file in os.listdir(src_example):
        img = torch.unsqueeze(torch.tensor(np.asarray(np.load(src_example, 'r+'), dtype=np.float32)), 0).to(device)
        with torch.no_grad():
            output_1 = model_1(img)
            output_2 = model_2(img)
